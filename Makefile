...
compile: 
	...
	javac $(JAVA_BUILD_OPTS) -cp ".:lib/*" -d build/classes/main `find src/main -name \*.java`
	javac $(JAVA_BUILD_OPTS) -cp ".:lib/*:build/classes/main" -d build/classes/main `find src/test -name \*.java`
...
test: compile
	...
	jar -cfm build/Test.jar def.manifest -C build/classes/main org
...
.PHONY: jni
jni: compile
	javah -jni -classpath build/classes/main -d build/jniheaders org.peergos.crypto.JniTweetNacl
